...
async def _send_reply(http: httpx.AsyncClient, chat_id: str, text: str) -> None:
    """發送訊息：優先使用 main.reply_text，失敗則使用 lark_client.send_text_to_chat"""
    try:
        if reply_text:
...
def _yesterday_range(tz: str | ZoneInfo = DEFAULT_TZ) -> tuple[datetime, datetime]:
    """計算當地時區的昨日 00:00 ~ 今日 00:00 區間"""
    tz_obj = ZoneInfo(str(tz)) if isinstance(tz, str) else tz
    now = datetime.now(tz_obj)
    today = now.date()
    yesterday = today - timedelta(days=1)
    start = datetime.combine(yesterday, dt_time.min, tzinfo=tz_obj)
...
async def record_message(event: Dict[str, Any]) -> None:
    """
    從 Lark 事件中記錄文字訊息到資料庫。僅對文字型別做保存，不觸發摘要指令。
    """
    try:
        ...
        if msg_type == "text" and text:
            async with AsyncSessionFactory() as db:
                await crud.upsert_chat(db, chat_id, None)  # 確保群組存在
...
async def maybe_handle_summary_command(event: Dict[str, Any]) -> None:
    """
    處理群內的 #summary 指令。支持 once / off / on / at / tz / lang 等子命令。
    """
    try:
        ev = event.get("event", {}) or {}
        msg = ev.get("message", {}) or {}
